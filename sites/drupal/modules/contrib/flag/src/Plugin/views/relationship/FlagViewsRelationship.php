...
//        ];
        if ($session_flaggings = \Drupal::request()->getSession()->get('flaggings', [])) {
          // Add in the flagging_[table] id from the Session for
          // anonymous users, assuring they only see their own flags.
          $or = new Condition('OR');
          foreach ($session_flaggings as $session_flag) {
            $or->condition('flagging_' . $this->options['table'] . '.id', $session_flag, '=');
          }
          $this->query->addWhere(0, $or);
        } else {
          // If no flaggings are in the Session, nothing should be shown.
          // Since flagging_[table] id can't be NULL, adding this in
          // the WHERE assures nothing is returned.
          $this->query->addWhere(0, 'flagging_' . $this->options['table'] . '.id');
        }
      }
    }
